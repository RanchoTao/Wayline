...
export function buildWaylineDemo(now = Date.now()): { projects: WaylineProject[]; tasks: WaylineTask[] } {
  ...
  const deadline = new Date(now + 8 * DAY_MS).toISOString();
  ...
  const rows = [
    ["定义核心闭环与验收条件", 10, 90, -1, 100],
    ["统一 Project / Task / Capture 数据模型", 10, 150, 0, 100],
    ["实现 VisualDeadline Priority Engine", 10, 180, 1, 35],
    ["完成 Unified Capture adapter", 9, 180, 1, 20],
    ["完成多模态确认流程", 8, 120, 3, 0],
    ["补齐计划矩阵与任务列表", 8, 150, 2, 0],
    ["生成真实数据回顾", 8, 120, 1, 0],
    ["跑通端到端黑客松演示", 10, 120, 6, 0],
  ] as const;
  const tasks = rows.map(([title, importance, estimatedMinutes, dependencyIndex, progress], index): WaylineTask => ({
    id: `demo-task-${index + 1}`,
    title,
    description: index === 2 ? "保持 VisualDeadline importance-urgency-v1 行为不变。" : undefined,
    projectId: project.id,
    createdAt,
    deadline: new Date(now + Math.max(1, index + 1) * DAY_MS).toISOString(),
    importance,
    estimatedMinutes,
