...
export function TimeMetrics() {
  ...
  if (!project || !m) {
    return (
      <div className="flex h-14 shrink-0 items-center justify-center border-b border-line bg-card">
        <span className="vd-label">MAKE TIME VISIBLE — ENTER A GOAL OR LOAD THE DEMO</span>
      </div>
    );
  }
  ...
  return (
    <div className="grid shrink-0 grid-cols-2 gap-x-6 gap-y-2 border-b border-line bg-card px-4 py-2.5 md:grid-cols-5">
      <div>
        <div className="vd-label mb-0.5">Calendar time</div>
        <div className="vd-num text-[15px] font-bold text-ink">
          {formatHours(m.remainingMs / 3_600_000)}{" "}
          <span className="text-[11px] font-medium text-muted">remaining</span>
        </div>
      </div>
      <div>
        <div className="vd-label mb-0.5">Effective time</div>
        <div className="vd-num text-[15px] font-bold text-ink">
          {m.availableHours.toFixed(1)}h{" "}
          <span className="text-[11px] font-medium text-muted">of work left</span>
        </div>
      </div>
      <div>
        <div className="vd-label mb-0.5">Time used</div>
      ...
      <div>
        <div className="vd-label mb-0.5">Work done</div>
      ...
      <div className="col-span-2 md:col-span-1">
        <div className="vd-label mb-0.5">Schedule gap</div>
        ...
        <div className="vd-num mt-0.5 text-[10px] text-muted">
          {Math.abs(m.scheduleGapHours).toFixed(1)}h of work {behind ? "off pace" : "banked"}
        </div>
      </div>
    </div>
  );
